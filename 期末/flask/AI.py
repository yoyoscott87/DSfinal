...
def weather():
    ...
    data = request.json
    location_name = data.get('city')  # 获取城市名
...
prompt = ""
for row in data:
    prompt += f"這是一串地點的天氣狀況和冷氣及除濕機狀況，地點{row['地點']}時間{row['時間']}，溫度{row['溫度(°C)']}，濕度{row['濕度']}，空氣品質{row['空氣品質']}，冷氣狀況是{row['冷氣狀況(12燈)']}，除濕機狀況是{row['除濕機狀況(16燈)']}；"
    ...
    try:
        if temperature is not None:
            if temperature > 23:
                ...
                print(response_text_on)
                print("溫度超過 20 度，開啟冷氣")
                return "開啟"
            else:
                response_text_off = control_device("off", "12")
                print(response_text_off)
                print("溫度低於等於 20 度，關閉冷氣")
                return "關閉"
    except Exception as e:
        print("控制冷氣出錯：", e)
...
#@app.route('/search-button',method = ['POST'])
